...
describe('Export Routes', () => {
  ...
  beforeEach(async () => {
    ...
    vi.doMock('../../server/services/fileService.js', () => ({
      readChapter: vi.fn().mockResolvedValue('Chapter content here'),
      writeChapter: vi.fn(),
      writeVersion: vi.fn(),
    ...
    // Seed a project with a chapter
    const projRes = await request(app)
      .post('/api/projects')
      .send({ name: 'Export Test Novel' });
  ...
  describe('GET /api/projects/:projectId/export/txt', () => {
    it('returns text with chapter content', async () => {
      const res = await request(app)
        .get(`/api/projects/${projectId}/export/txt`);

      expect(res.status).toBe(200);
      expect(res.text).toContain('Chapter One');
      expect(res.text).toContain('Chapter content here');
    });
    ...
    it('returns 404 when no chapters exist', async () => {
      // Create a new project without chapters
      const emptyRes = await request(app)
        .post('/api/projects')
        .send({ name: 'Empty Project' });
      const emptyProjectId = emptyRes.body.data.id;

      const res = await request(app)
        .get(`/api/projects/${emptyProjectId}/export/txt`);

      expect(res.status).toBe(404);
      expect(res.body.success).toBe(false);
    });

    it('includes volume titles when chapters are grouped', async () => {
      // Create a volume and assign the chapter to it
      const volRes = await request(app)
        .post(`/api/projects/${projectId}/volumes`)
        .send({ title: 'Arc One: The Beginning' });
      const volumeId = volRes.body.data.id;

      // Update chapter to belong to the volume
      await request(app)
      ...
      expect(res.status).toBe(200);
      expect(res.text).toContain('Arc One: The Beginning');
      expect(res.text).toContain('Chapter One');
    });
  });

  describe('GET /api/projects/:projectId/export/md', () => {
    it('returns markdown', async () => {
      const res = await request(app)
        .get(`/api/projects/${projectId}/export/md`);
  ...
  describe('GET /api/projects/:projectId/export/epub', () => {
    it('returns binary EPUB', async () => {
      const res = await request(app)
  ...
  describe('GET /api/projects/:projectId/export/docx', () => {
    it('returns binary DOCX', async () => {
      const res = await request(app)
        .get(`/api/projects/${projectId}/export/docx`);
  ...
  describe('GET /api/projects/:projectId/export/pdf', () => {
    it('returns binary PDF', async () => {
      const res = await request(app)
